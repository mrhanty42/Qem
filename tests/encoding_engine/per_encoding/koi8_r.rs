...
//
// Class A encoding: KOI8-R (legacy Russian Cyrillic single-byte).
//
//
// This file ships the fixed four-test contract for the
// `KOI8-R` Class A encoding. Each test creates its own fresh fixture
...
const LABEL: &str = "KOI8-R";

const LINE0: &str = "TARGET line0";
const LINE1: &str = "ascii line1";
/// Line containing the encoding-specific non-ASCII payload. `привет`
/// is a six-letter Russian word; every glyph is representable in
/// `KOI8-R` and round-trips cleanly through
/// `encoding_rs::KOI8_R::encode`.
const LINE2_TEXT: &str = "привет";
...
fn fixture_bytes() -> Vec<u8> {
    ...
    let (encoded, used, had_errors) = KOI8_R.encode(&text);
    assert_eq!(used, KOI8_R, "fixture must round-trip through {LABEL}");
...
fn opens_and_indexes_lines() {
    let dir = fresh_test_dir("per_encoding_koi8_r_open");
    ...
    let doc = Document::open_with_encoding(&path, encoding)
        .expect("open_with_encoding(KOI8-R) must succeed for valid fixture");
...
fn viewport_first_and_last_window() {
    let dir = fresh_test_dir("per_encoding_koi8_r_viewport");
    let path = dir.join("fixture.txt");
    let bytes = fixture_bytes();
    std::fs::write(&path, &bytes).expect("write fixture");
...
fn literal_and_regex_search_finds_known_match() {
    let dir = fresh_test_dir("per_encoding_koi8_r_search");
    let path = dir.join("fixture.txt");
    let bytes = fixture_bytes();
    std::fs::write(&path, &bytes).expect("write fixture");
...
fn save_round_trip_no_edits() {
    let dir = fresh_test_dir("per_encoding_koi8_r_save");
    let path = dir.join("fixture.txt");
...
/// `Привет` is six Cyrillic letters fully representable in `KOI8-R`.
const NON_ASCII_INSERT: &str = "Привет";

#[test]
fn edit_and_save_round_trip() {
 // — fifth contract. Validates (edit + save
 // round-trip) and (save fidelity) for `KOI8-R` through the
 ...
 // text round-trips byte-identically against the in-memory document.
    let dir = fresh_test_dir("per_encoding_koi8_r_edit");
    let path = dir.join("fixture.txt");
    let saved = dir.join("fixture.saved.txt");
    let bytes = fixture_bytes();
    std::fs::write(&path, &bytes).expect("write fixture");
